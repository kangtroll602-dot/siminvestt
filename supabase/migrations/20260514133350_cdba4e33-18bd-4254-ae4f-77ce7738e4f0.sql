...
create policy "Admins delete chat messages" on public.chat_messages for delete to authenticated using (public.has_role(auth.uid(), 'admin'::public.app_role));
...
create policy "Public read chat attachments" on storage.objects for select to anon, authenticated using (bucket_id = 'chat-attachments');
...
create policy "Admins upload chat attachments" on storage.objects for insert to authenticated with check (bucket_id = 'chat-attachments' AND public.has_role(auth.uid(), 'admin'::public.app_role));
create policy "Admins manage site assets" on storage.objects for all using (bucket_id = 'site-assets' and public.has_role(auth.uid(), 'admin')) with check (bucket_id = 'site-assets' and public.has_role(auth.uid(), 'admin'));
